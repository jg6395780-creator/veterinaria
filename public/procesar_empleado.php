...
if ($action === 'editar') {
    ...
    try {
        if ($pass !== '') {
            ...
            $pdo->prepare("UPDATE usuarios SET nombre_completo=:n, rol=:r, password=:p WHERE id=:id")
                ->execute([':n'=>$nombre, ':r'=>$rol, ':p'=>$hash, ':id'=>$id]);
        } else {
            $pdo->prepare("UPDATE usuarios SET nombre_completo=:n, rol=:r WHERE id=:id")
                ->execute([':n'=>$nombre, ':r'=>$rol, ':id'=>$id]);
        }
